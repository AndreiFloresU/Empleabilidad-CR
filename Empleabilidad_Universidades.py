...
def calcular_empleabilidad_por_cohorte(df_grad, df_lab, cedulas_validas):
    """
    Calcula la tasa de empleabilidad por cohorte para la universidad seleccionada.
    """
    # Filtrar solo graduados válidos
    df_grad_valido = df_grad[df_grad["cedula"].isin(cedulas_validas)]

    # Contar graduados por cohorte
    graduados_por_cohorte = (
        df_grad_valido.groupby(["universidad", "anio_graduacion"])["cedula"]
        .nunique()
        .reset_index()
    )
    graduados_por_cohorte.columns = [
        "universidad",
        "anio_graduacion",
        "total_graduados",
    ]
    ...
    df_lab_valido = df_lab[df_lab["cedula"].isin(cedulas_validas)]

    # Unir con datos de graduados para obtener cohorte
    df_empleados = df_lab_valido.merge(
        df_grad_valido[["cedula", "universidad", "anio_graduacion"]],
        on="cedula",
        how="left",
    )
    ...
    empleados_por_cohorte = (
        df_empleados.groupby(["universidad", "anio_graduacion"])["cedula"]
        .nunique()
        .reset_index()
    )
    empleados_por_cohorte.columns = [
        "universidad",
        "anio_graduacion",
        "total_empleados",
    ]
    ...
    resultado = graduados_por_cohorte.merge(
        empleados_por_cohorte, on=["universidad", "anio_graduacion"], how="left"
    )
